 ...
 *
 * sound.h should look like this:
 *     const int sound_length=10000;
 ...
 * wav2c macstartup-cut.wav sound.h sound
 *
...
ISR(TIMER1_COMPA_vect) {
    if (sample >= sound_length) {
        if (sample == sound_length + lastSample) {
            stopPlayback();
        }
        else {
            if(speakerPin==11){
                // Ramp down to zero to reduce the click at the end of playback.
                OCR2A = sound_length + lastSample - sample;
            } else {
                OCR2B = sound_length + lastSample - sample;
            }
        }
    }
    else {
